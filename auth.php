...
class auth_plugin_authphpbb3 extends DokuWiki_Auth_Plugin {
	...
	public function retrieveUsers($first = 0, $limit = 10, $filter = array()) {
		...
		foreach($filter as $item => $pattern) {
			$likeexpr = str_replace (array ("?", "*"), array ("_", "%"), escape ($pattern) );
			if($item == 'user') {
				$whereexpr .= (($cnt++ > 0) ? "AND" : "WHERE");
				$whereexpr .= " lower(u.username) like lower(" . $likeexpr . ") ";
			} elseif ($item == 'name') {
				$whereexpr .= (($cnt++ > 0) ? "AND" : "WHERE");
				if ($realnamefield == "pf_"){
					$whereexpr .= " lower(u.username) like lower(" . $likeexpr . ") ";
				} else {
					$whereexpr .= " lower(pf.{$realnamefield}) like lower(" . $likeexpr . ") ";
				}
			} elseif ($item == 'mail') {
				$whereexpr .= (($cnt++ > 0) ? "AND" : "WHERE");
				$whereexpr .= " lower(u.user_email) like lower(" . $likeexpr . ") ";
			} elseif ($item == 'grps') {
				$whereexpr .= (($cnt++ > 0) ? "AND" : "WHERE");
				$whereexpr .= " lower(g.group_name) like lower(" . $likeexpr . ") ";
			}
		}
		...
		while($row = mysql_fetch_array($rs)) 
			{
				// take first username
				if (!$user_first) {
					$user_first = $row['username'];
				}
			};
		$user_last = $row['username'];
		
		// clear result
		mysql_free_result ($rs);
		unset($row);

		$need_and = ($whereexpr? "AND" : "WHERE");
		if ($realnamefield == "pf_"){
			$query = "select u.username, u.user_email, g.group_name
						from {$this->phpbb3_table_prefix}users u, {$this->phpbb3_table_prefix}groups g, {$this->phpbb3_table_prefix}user_group ug "
						. $whereexpr . $need_and . "u.username >= {$user_first} AND u.username <= {$user_last} 
						order by u.username";
		} else{ 
			$query = "select u.username, u.user_email, g.group_name, pf.{$realnamefield}
						from {$this->phpbb3_table_prefix}users u, {$this->phpbb3_table_prefix}groups g, {$this->phpbb3_table_prefix}user_group ug, {$this->phpbb3_table_prefix}profile_fields_data pf "
						. $whereexpr . $need_and . "where u.username >= {$user_first} AND u.username <= {$user_last} 
						order by u.username";
		};		
	
		$rs = mysql_query($query, $link );
		$current_userid = 0;   //  real user can't have this id
		while($row = mysql_fetch_array($rs)) 
			{
				if ($row['user_id'] != $current_userid){
					if ($current_userid != 0){
						$this_user_unfo['grps'] = $this_grouplist;
						$tmpvar[] = $this_user_unfo;
					}
					$current_userid = $row['user_id'];
					$this_user_unfo['user'] = $row['username'];
					$this_user_unfo['mail'] = $row['user_email'];
					$this_user_unfo['name'] = ( ($realnamefield == "pf_") ? $row['username'] : $row["{$realnamefield}"] );
					$this_grouplist[] = $row['group_name'];
				} else {
					$this_grouplist[] = $row['group_name'];
				}
			};	
    ...
    public function getUserCount($filter = array()) {
		...
		$rs = mysql_query($query, $link );
		dbglog("authphpbb3 error: why boolean?  ");
		dbglog("query is \"{$query} \"");
		dbglog("result is \"{$rs} \"");
